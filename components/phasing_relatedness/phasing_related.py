...
def phase_variant_multiallelic(patient, mother, father, phased_variants, unphased_variant_name, phased_variant_name):
    ...
    if father_state != 1: 
        father_phased['I'], father_phased['N'] = father_state // 2, father_state // 2
    ...
    if patient_state < min_patient_state: # variant absent in patient, most probable configuration can be suggested in some cases 
        if (mother_state == 2) & (len(father_phased) > 0):
            ...
            logging.warning(f'{unphased_variant_name} genotype in child is {patient}, parents are {father}, {mother}. One of the genotypes is incorrect. Additional checks are required')
            return phased_variants
    ...
    if patient_state > max_patient_state: # de novo variant appearance; 2/1 anynot1 anyone1 was already handled by previous logic. Other can't be solved
        phased_variants.pop(phased_variant_name)
        logging.warning(f'{unphased_variant_name} genotype in child is {patient}, parents are {father}, {mother}. Variant is de novo or one of the genotypes is incorrect. Additional checks are required')
        return phased_variants
    ...
    if len(ids_list) == 2:  # deletion in duo, always assume it is inhereted 
        ...
        if father_mismatch > mother_mismatch: 
            ...
            if ploidy[father_id] == 1: 
                ...
                deletion_source = mother_id
            elif father_mismatch == 0: 
                logging.warning(f'Source of deleted copy in {patient_id} not identified. Patient genotype can be explained with parents variants. Might be false positive deletion call. Processing as diploid.')
                return False, 'no' 
            else:
...
def phase_trio_mode(patient_id, mother_id, father_id, genotypes_extracted, all_variants, deletion_source, is_patient_with_deletion): 
    ...
    phased_variants = {}
    for variant_id in all_variants: 
        phased_variants = phase_variant(return_genotype(variant_id, genotypes_extracted[patient_id], is_patient_with_deletion=is_patient_with_deletion),
    ...
    for group_id, count in multi_groups.items():
        ...
        phased_variants = phase_variant_multiallelic(return_genotype(unphased, genotypes_extracted[patient_id],
                                                                     is_patient_with_deletion=is_patient_with_deletion),
                                            return_genotype(unphased, genotypes_extracted[mother_id], 
                                                            is_deletion_source=(mother_id == deletion_source)),
                                            return_genotype(unphased, genotypes_extracted[father_id],
...
def phase_duo_mode(patient_id, parent_id, genotypes_extracted, all_variants, deletion_source, is_patient_with_deletion):
    ...
    phased_variants = {}
    for variant_id in all_variants: 
            phased_variants = phase_variant_duo(return_genotype(variant_id, genotypes_extracted[patient_id],
                                                                is_patient_with_deletion=is_patient_with_deletion),
    ...
    for block_id, row in ped_file.iterrows(): 
        ...
        if (mother_id != 'proxy') & (father_id != 'proxy'):
            ...
            if pl_ch == 'match copy strategy':
                ...
                is_patient_with_deletion, deletion_source = determine_deletions_source([patient_id, mother_id, father_id], genotypes_extracted, all_variants, ploidy)
            else: 
            ...
            if pl_ch == 'match copy strategy':
                # find deletion source and consider no variants inherited from deletion source 
                is_patient_with_deletion, deletion_source = determine_deletions_source([patient_id, parent_id], genotypes_extracted, all_variants, ploidy)
            else: 
                deletion_source, is_patient_with_deletion = 'no', False
